...
- 对于支付宝（alipay），通常从包含表头的一行开始，后面每一行都是一笔交易，重点字段包括：交易创建时间/付款时间（日期时间）、收/支（支出/收入/不计收支）、金额（元）、交易对方、商品名称、收/付款方式、备注、交易状态、资金状态、订单号/商户订单号等。
- 【支付宝 · 退款与净额 · 硬性规则】当 channel = "alipay" 时还必须遵守：
  1. **先关联再输出**：同一段 rawContent 里常同时出现「原支付」与「退款成功/退款到账」等多行。必须根据订单号、商户订单号、交易对方、日期时间 proximity、金额对应关系，把同一笔订单的支付与退款**配对**后再决定输出什么；**禁止**只解析支付行却忽略退款行。
  2. **全额退款**：若可判定该笔消费已**全额**退回（例如存在「退款成功」且累计退款金额等于原支付金额，或交易状态/商品说明已表明整单退款），则**净收支必须为 0**。**优先**：该订单**不生成任何** transaction（既不输出原 expense，也不把退款单独记成 income，以免重复）。**严禁**只输出原支付的 expense 而不处理对应退款流水。
  3. **部分退款**：必须在账务上扣掉已退金额。**任选一种一致做法**（优先 a）：（a）只输出一笔 expense，amount = 原支付金额减去本账单内已成功退款金额的**差额**（净支出），note 可写「含部分退款」；（b）输出一笔原金额 expense + 一笔 type 为 income 的退款入账，amount 为退款额，note 标明「退款」。**严禁**仍按原支付全额记支出却漏掉退款行。
  4. **状态与关闭**：遇「交易关闭」「已退款」「退款成功」等，须结合收/支方向与金额判断是否仍应记支出；**禁止**将已全额退款完成的订单仍记为有效支出。
  5. **闲鱼/二手卖出等收款**：当「收/支」为「收入」且交易状态为「交易成功」（或等价有效成功状态）、且交易对方、商品说明或备注可判断为闲鱼/二手平台**卖出收款**时，**必须**生成对应的 income 记录；category 优先使用收入类目列表中的「闲鱼」（若列表无该词则用「其他」或「待确认」），merchant 可用「闲鱼」或对方名称；**禁止**因金额小、描述像个人转让而整行漏记。
...
- 若交易文本出现「住宿」「酒店」「宾馆」「旅店」「民宿」「客栈」且该笔为支出，则 category 必须归类为「生活」。
...
- 【招商银行特殊规则 · 跳过重复流水】当 channel = "cmb" 时，以下类型的交易**必须跳过**（不生成 transaction 记录），因为它们在其他渠道的明细中已有记录（用户会单独导入微信账单、支付宝账单，勿在招行流水中再记一笔）：
  ...
  4. 摘要或对方户名含「快捷支付」「银行快捷支付」或与二者同义的银行侧快捷扣款描述——这是支付宝/微信等三方支付从本行卡扣款的通道流水，**逐笔消费已在支付宝流水或微信流水中体现**，本笔**必须跳过**，禁止再生成 transaction。**例外**：若摘要或对方户名同时含「上海蔚来融资租赁有限公司」或「蔚来融资租赁」等可明确指向该公司的表述，则**不得**因「快捷支付」而跳过——此类多为车贷/融资租赁还款，微信与支付宝流水通常无对应明细，须**正常生成 transaction**。
